...
export const SETTING_DOMAIN_NAME_KEY = 'sandboxea8656ef81c944c7806eec0d6188fea3.mailgun.org';
export const SETTING_API_KEY_NAME = 'key-d145174e9c461210a4f4c8c802daab0e';
...
export class MailProviderMailgunService implements IMailProvider {
  ...
  public async sendMail(from: string, to: string, cc:string, bcc: string, subject : string, message: string) : Promise<boolean> {
    if(cc===null || cc=== undefined || cc==="")
    {
      cc= null;
    }
    if(bcc===null || bcc=== undefined || bcc==="")
    {
      bcc= null;
    }
    const body = this._getSendMailRequestBody(from, to, cc, bcc, subject, message);
  ...
  private _getSendMailRequestBody(from : string, to : string, cc: string, bcc:string, subject : string, message : string) {
